...
def render_paper_log() -> None:
    st.header("Paper Log")
    st.caption("Review paper signal events generated by the strategy engine.")

    if not PAPER_LOG_PATH.exists():
        st.info("No paper log found yet. Run a signal report to create paper events.")
        return

    paper = pd.read_csv(PAPER_LOG_PATH)

    if paper.empty:
        st.info("The paper log exists, but it does not contain any events yet.")
        return
